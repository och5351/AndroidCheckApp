...
if($_SESSION['type']=='교수') {
    ...
    $name = $_SESSION['name'];
    $result=$PDO->query("SELECT * FROM web.class WHERE user_id = '$user_id' and name = '$name'")->fetchAll();
...
<div id="main" class="all">
    ...
    <div class="middle">
        ...
        <p class="relative"><?=$result[$i]["class_name"]?>_<?=$result[$i]['division']?></p>
        <p class="class_name" style="display: none"><?=$result[$i]['class_name']?></p>
        <p class="division" style="display: none"><?=$result[$i]['division']?></p>
        <hr>
        <br>
        <br>
    </div>
